...
// unsafe to inline into generated JavaScript: `<`/`>`/`&` (so a `</script>`
// sequence can't break out of an inline <script> tag) and the U+2028/U+2029
// line separators (historically invalid inside JS string literals). We must NOT
// escape `\` or control characters here, since `JSON.stringify` already escapes
// those correctly and re-escaping them would produce invalid output.
const UNSAFE_CODE_INJECTION_CHAR_MAP: Record<string, string> = {
  '&': '\\u0026',
  '<': '\\u003C',
  '>': '\\u003E',
  '\u2028': '\\u2028',
...
function escapeUnsafeCharsForCodeInjection(str: string): string {
  return str.replace(/[&<>\u2028\u2029]/g, char => UNSAFE_CODE_INJECTION_CHAR_MAP[char] || char);
}
...
export function generateReleaseInjectorCode({
  ...
  let code = `e.SENTRY_RELEASE={id:${escapeUnsafeCharsForCodeInjection(JSON.stringify(release))}};`;

  if (injectBuildInformation) {
    const buildInfo = getBuildInformation();

    code += `e.SENTRY_BUILD_INFO=${escapeUnsafeCharsForCodeInjection(JSON.stringify(buildInfo))};`;
  }
...
export function generateModuleMetadataInjectorCode(metadata: any): CodeInjection {
  ...
  return new CodeInjection(
    `e._sentryModuleMetadata=e._sentryModuleMetadata||{},e._sentryModuleMetadata[(new e.Error).stack]=function(e){for(var n=1;n<arguments.length;n++){var a=arguments[n];if(null!=a)for(var t in a)a.hasOwnProperty(t)&&(e[t]=a[t])}return e}({},e._sentryModuleMetadata[(new e.Error).stack],${escapeUnsafeCharsForCodeInjection(
